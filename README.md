...
輸出 JSON 包含版本、metadata 目錄路徑、工作目錄、是否偵測到本地 metadata manifest。

### 1. 下載 data.gov.tw 匯出檔

```bash
tw-odc metadata download                            # 預設只下載 JSON 格式
tw-odc metadata download --all                      # 下載所有格式（JSON/CSV/XML）
tw-odc metadata download --only export-json.json    # 只下載一個檔案
tw-odc metadata download --no-cache                 # 忽略 ETag 快取
tw-odc metadata download --dir /path/to/dir         # 指定 metadata 目錄
```

預設只下載 JSON 格式的匯出檔（含每日異動），使用 `--all` 下載全部格式。首次執行時會自動從內建的 `default_manifest.json` 建立 metadata manifest。

Metadata 存放位置依以下優先順序決定：
1. `$PWD` 存在 `manifest.json` 且 `type == "metadata"` → 使用 `$PWD`
2. 否則 → `~/.config/tw-odc/`（跨平台設定目錄）
3. 可用 `--dir` 手動指定

### 1b. 每日異動更新

```bash
# 一鍵完成：下載 JSON metadata + 套用異動
make daily

# 或分步操作：
tw-odc metadata download                            # 下載 JSON metadata（含每日異動）
tw-odc metadata apply-daily                          # 套用異動到 provider manifests
tw-odc metadata apply-daily --date 2026-03-10        # 指定日期

# 下載非 JSON 格式的每日異動
tw-odc metadata download --only daily-changed-csv.csv --date 2026-03-10
```

`apply-daily` 讀取 `daily-changed-json.json`，將異動資料集合併進已存在的 provider manifest。若遇到尚未建立的 provider，會自動從 `export-json.json` 建立 manifest（auto-scaffold）。輸出 JSON 摘要（created / updated / skipped / warnings）。
...
```
tw-odc/
├── manifest.json              # type: metadata（data.gov.tw 匯出 URL，開發用）
├── src/tw_odc/                # CLI 套件（src layout）
│   ├── __init__.py            # FORMAT_ALIASES（中文格式名對照）
│   ├── __main__.py            # python -m tw_odc 進入點
│   ├── cli.py                 # typer app，metadata/dataset/config 子命令
│   ├── paths.py               # 跨平台路徑解析（data_dir, ensure_manifest）
│   ├── fetcher.py             # 非同期下載（aiohttp, etag 快取, HEAD 健康檢查）
...
│   └── default_manifest.json  # 內建預設 metadata manifest（首次自動部署）
...
增量更新：`metadata download → metadata apply-daily`（或 `make daily`）
...
tw-odc 目前僅支援[政府資料開放平臺](https://data.gov.tw/)的資料集，**不支援**[政府電子採購網](https://web.pcc.gov.tw/)（Government e-Procurement System）。

政府電子採購網由行政院公共工程委員會維運，提供招標公告、決標公告、廠商查詢（含優良廠商、拒絕往來廠商）、採購預告、採購統計等功能。若需查詢特定廠商的政府標案得標紀錄、決標金額、或廠商資格等資訊，請直接前往該網站查詢。

> **備註**：data.gov.tw 上雖有行政院公共工程委員會提供的多項採購統計資料集（如「巨額採購廠商名單」、「廠商分析資料」、「決標金額前5大」等），但這些資料集的下載端點 (`web.pcc.gov.tw`、`pcic.pcc.gov.tw`) 存在 SSL 憑證問題，可能無法正常下載。

### 社群替代方案：開放政府採購 API

感謝 [Ronny Wang](https://github.com/ronnywang) 開發的[政府採購資料 API](https://pcc-api.openfun.app/)，將政府電子採購網的公開招標資訊重新整理並提供開放 API（支援 CORS）。若官方網站不便查詢，可利用此站進行查詢。
...
- src layout（`src/tw_odc/`）與 `uv_build` 建置
- 跨平台 metadata 存放（`~/.config/tw-odc/`），首次執行自動 bootstrap
- `config show` 子命令，查看版本與路徑資訊
- `metadata` 子命令支援 `--dir` 指定 metadata 目錄
- 下載前 HEAD 健康檢查
- 格式偵測與驗證（inspector）
- 五星評分模型（scorer，★1–★3，★4/★5 規劃中）
- 政府資料品質指標評分（gov-tw scorer）
- 每日異動資料集下載（`params` URL 模板）與增量更新（`apply-daily`，含 auto-scaffold）
- 原始資料內容查看（`dataset view`）
- 國際化（i18n，en / zh-TW）
- Metadata 關鍵字搜尋（`metadata search`，slim JSONL 索引）
